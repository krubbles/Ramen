...
public static class AgentTrainingExtensions
{
    ...
    public static Move SelectBestMoveMonteCarlo(this RamenAgent agent, List<(Move move, float probability)> candidateMoves, int samples, float temp)
    {
        ...
        foreach (var (candidateMove, _) in candidateMoves)
        {
            ...
            agent.GameState.MoveState.RevertToStep(agent.GameState.MoveState.MoveStep - 1);
        }
